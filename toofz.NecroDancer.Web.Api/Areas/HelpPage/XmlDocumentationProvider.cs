    ...
    public class XmlDocumentationProvider : IDocumentationProvider, IModelDocumentationProvider
    {
        ...
        public IEnumerable<HttpStatusCodeDescription> GetHttpStatusCodeDescriptions(HttpActionDescriptor actionDescriptor)
        {
            ...
            return httpStatusCodes.OfType<XPathNavigator>().Select(e =>
            {
                ...
                var i = cref.LastIndexOf('.');
                var status = cref.Substring(i + 1);
                var statusCode = (HttpStatusCode)Enum.Parse(typeof(HttpStatusCode), status);
                var message = e.Value.Trim();

                return new HttpStatusCodeDescription(statusCode, message);
            });
        }

        public string GetDocumentation(MemberInfo member)
        {
            string memberName = String.Format(CultureInfo.InvariantCulture, "{0}.{1}", GetGenericTypeName(member.DeclaringType), member.Name);
